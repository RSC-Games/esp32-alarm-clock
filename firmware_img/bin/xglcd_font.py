...
class XglcdFont(object):
    ...
    def __init__(self, path: str, width: int, height: int, start_letter=32, letter_count=96):
        """
    ...
    def __load_xglcd_font(self, path: str) -> None:
        """
    ...
    def get_letter(self, letter: str, c=1, rotate=0) -> tuple[FrameBuffer | None, int, int]:
        ...
        for i in range(0, array_size, byte_height):
            ...
            if byte_height > 1:
                ba2[pos + width] = ba[i + 1] ^ inv_val
            if byte_height > 2:
                ba2[pos + width * 2] = ba[i + 2] ^ inv_val
            if byte_height > 3:
                ba2[pos + width * 3] = ba[i + 3] ^ inv_val
            if byte_height > 4:
                ba2[pos + width * 4] = ba[i + 4] ^ inv_val
            if byte_height > 5:
                ba2[pos + width * 5] = ba[i + 5] ^ inv_val
            pos += 1
