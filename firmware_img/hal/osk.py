...
def prompt_text(key_layout: int, max_chars: int, hide_text=False) -> str:
    ...
    if key_layout == 0:
        return _do_osk_input(["UPPERCASE", "LOWERCASE", "SYMBOLS"], qwerty_key_set, max_chars, hide_text)       
        
    # Use the numpad in this keyboard layout.
    elif key_layout == 1:
        return _do_osk_input(["PRIMARY"], numpad_key_set, max_chars, hide_text)
        
    else:
        raise ValueError("invalid osk layout requested")
...
def _do_osk_input(subcharsets: list[str], key_set: dict[str, list], max_chars: int, hide_text: bool) -> str:
    ...
    while True:
        ...
        if peripherals.get_button_wait(peripherals.BTN_CONFIRM):                
            ...
            if cur_char[1] == "RETURN":
                # Return the list of characters after making it a string.
                peripherals.wait_buttons_all_released()
                return "".join(char_string)
            
            elif cur_char[1] == "SHIFT":
        ...
        elif peripherals.get_button_wait(peripherals.BTN_BACK):
            # Text entry exited
        ...
        elif peripherals.get_button_wait(peripherals.BTN_DIR_UP):
...
def prompt_yn(label: str, prompt: list[str]) -> bool:
    ...
    while True:
        ...
        elif peripherals.get_button_wait(peripherals.BTN_DIR_UP) or \
            peripherals.get_button_wait(peripherals.BTN_DIR_DOWN):
