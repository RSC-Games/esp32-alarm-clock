...
class Assembler:
    ...
    def fill(self, section, amount, fill_byte):
        ...
        # XXX: We are filling the text section with ZEROS AND THERES NOTHING THAT
    ...
    def d_fill(self, amount, size: str, fill_bytes: str):
        # XXX: Hardcoded for text section (assuming d_XXX is for text?)
        
        fill = int(self.opcodes.eval_arg(fill_bytes)).to_bytes(int(size), 'little') * int(amount)
        self.offsets['text'] += len(fill)
        self.sections['text'].append(fill)
